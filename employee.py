...
class Employee:
    ...
    def AddData(self):
        ...
        try:
            cursor.execute(
                "insert into saumya values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
                (
                    self.department.get(),
                    self.name.get(),
                    self.phone.get(),
    ...
    def ShowAll(self):
        ...
        cursor = conn.cursor()
        try:
            cursor.execute("select * from saumya")
    ...
    def deleteData(self):
        ...
        try:
            cursor.execute(
                "delete from saumya where IdProof= %s", (self.idProof.get(),)
            )
            conn.commit()
            messagebox.showinfo(
    ...
    def searchEmployeeInfo(self):
        ...
        try:
            cursor.execute(
                "select * from saumya where " + str(self.searchBy.get()) + " LIKE %s",
                (self.searchText.get(),),
            )
            data = cursor.fetchall()
    ...
    def updateData(self):
        ...
        try:
            cursor.execute(
                "update saumya set Department=%s,Name=%s,Phone=%s,Email=%s,DOB=%s,DOJ=%s,Salary=%s,IdType=%s,Married=%s,Gender=%s,Address=%s,Country=%s where IdProof=%s",
                (
                    self.department.get(),
                    self.name.get(),
                    self.phone.get(),
